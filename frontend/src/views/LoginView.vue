...
<template>
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-violet-50 via-slate-50 to-cyan-50 dark:from-slate-950 dark:via-violet-950/20 dark:to-slate-950 p-4 transition-colors duration-300">
    ...
    <div class="pointer-events-none fixed inset-0 overflow-hidden" aria-hidden="true">
      <div class="absolute -top-40 -left-40 w-96 h-96 rounded-full bg-violet-500/10 blur-3xl dark:bg-violet-500/5"></div>
      <div class="absolute -bottom-40 -right-40 w-96 h-96 rounded-full bg-cyan-500/10 blur-3xl dark:bg-cyan-500/5"></div>
    </div>
    ...
    <div class="relative w-full max-w-sm">
      ...
      <div class="bg-white dark:bg-slate-900/70 dark:backdrop-blur-xl border border-slate-200 dark:border-slate-800/60 rounded-2xl shadow-xl dark:shadow-2xl dark:shadow-slate-950/50 p-8">
        <h1 class="text-xl font-bold text-slate-900 dark:text-slate-100 mb-1">Welcome back</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mb-7">Sign in to your workspace</p>
        ...
        <p class="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
          No account?
          <router-link to="/register" class="font-semibold text-violet-600 dark:text-violet-400 hover:text-violet-500 transition-colors">
            Create workspace
          </router-link>
        </p>
      </div>
